...
import { Leaf, HeartPulse, Dumbbell, Salad, TreePalm } from "lucide-react";
...
const TAG_KEYS: Record<string, string> = {
    "Keto friendly": "dietaryKeto",
    "Vegan": "dietaryVegan",
    "Muscle gain": "dietaryMuscle",
    "Diabetic safe": "dietaryDiabetic",
    "Palm oil free": "dietaryPalmFree",
    "Contains palm oil": "dietaryPalm",
};

export async function DietaryTags({ tags }: DietaryTagsProps) {
        ...
        <div className="flex flex-wrap gap-2 mt-4">
            {tags.map((tag) => (
                <Badge
                    key={tag}
                    variant="outline"
                    className={`flex items-center text-xs font-medium px-2.5 py-1 ${getTagStyles(tag)}`}
                >
                    {getTagIcon(tag)}
                    {TAG_KEYS[tag] ? t(TAG_KEYS[tag] as Parameters<typeof t>[0]) : tag}
                </Badge>
            ))}
        </div>
    );
